...
and why your map looks mostly blank ("you sure ask a lot of questions") and hang on did you just say the sky ("of course, where do you think
...
static INPUT: &str= "sixsrvldfour4seven
...
fn main() {
    // replace word numbers with digit numbers (part 2)
    // let re = Regex::new(r"(one|two|three|four|five|six|seven|eight|nine)").unwrap();
    // let input = re.replace_all(INPUT, |caps: &regex::Captures| {
    //     match &caps[0] {
    //         "one" => "1",
    //         "two" => "2",
    //         "three" => "3",
    ...
    .iter()
    .map(|element| {
        let first_digit = element.chars().find(|c| c.is_digit(10)).unwrap_or('0');
        let last_digit = element.chars().rev().find(|c| c.is_digit(10)).unwrap_or('0');
